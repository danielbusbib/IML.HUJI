...
class UnivariateGaussian:
    ...
    def noraml_pdf(self, x):
        """
        calculate pdf on x
        :param x: sample
        :return:
        """
        denom = (2 * math.pi * self.var_) ** 0.5
    ...
    def log_likelihood(mu: float, sigma: float, X: np.ndarray) -> float:
        ...
        un.mu_, un.var_, un.fitted_ = mu, sigma, True
        return sum(np.log(un.pdf(X)))
...
class MultivariateGaussian:
    ...
    def pdf(self, X: np.ndarray):
        ...
        if not self.fitted_:
            raise ValueError("Estimator must first be fitted before calling `pdf` function")
        a = []
        detCov, invCov = np.linalg.det(self.cov_), np.linalg.inv(self.cov_)
        p1 = pow(2 * math.pi, float(X.shape[1]) / 2) * pow(detCov, 0.5)
        for i in range(X.shape[0]):
            A = X[i] - self.mu_
            s = np.matmul(np.matmul(A, invCov), A.T)
            a.append(math.exp(-0.5 * s) / p1)
        return np.array(a)
    ...
    def log_likelihood(mu: np.ndarray, cov: np.ndarray, X: np.ndarray) -> float:
        ...
        mv.mu_, mv.cov_, mv.fitted_ = mu, cov, True
        return sum(np.log(mv.pdf(X)))
